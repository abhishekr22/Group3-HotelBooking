...
public class CustomerWebRestController {
	...
	@GetMapping("/")
	public List<Passengers> getCustomerList(HttpServletRequest req){
		HttpSession session=req.getSession(false);
		if(session!=null) {
	...
	@PostMapping("/save")
	public ResponseEntity<CustomerBasicInfoDTO> saveCustomer(@RequestBody @Valid CustomerSavePostDTO customer,HttpServletRequest req){
		HttpSession session=req.getSession(false);
		if(session!=null) {
